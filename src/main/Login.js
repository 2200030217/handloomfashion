...
export default function CustomerLogin({onCustomerLogin,onSellerLogin}) {
  ...
  const handleSubmit = async (e) => {
    ...
    try 
    {
      const customerresponse = await axios.post('http://localhost:2563/checkcustomerlogin', formData);
      const sellerresponse = await axios.post('http://localhost:2563/checksellerlogin', formData);
